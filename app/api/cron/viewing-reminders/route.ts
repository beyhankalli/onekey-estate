...
export async function GET(request: Request) {
  try {
    ...
    for (const booking of bookings || []) {
      ...
      if (booking.customer_email) {
        await sendTransactionalEmail({
          ...
          html: emailShell(`
            <h1 style="margin:0 0 12px;color:#1c3053;font-size:26px;">Viewing reminder</h1>
            <p style="margin:0 0 24px;color:#4b5563;line-height:1.7;">Dear ${escapeHtml(booking.customer_name)}, this is a reminder that your property viewing is scheduled for tomorrow.</p>
            <div style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:14px;padding:20px;line-height:1.8;">
              ...
              Address: ${escapeHtml(property?.full_address || "Please contact OneKey for the address")}
            </div>
          `),
        });
        sent += 1;
      ...
      if (agent?.email) {
        await sendTransactionalEmail({
          ...
          html: emailShell(`
            <h1 style="margin:0 0 12px;color:#1c3053;font-size:26px;">Viewing reminder</h1>
            <p style="margin:0 0 24px;color:#4b5563;line-height:1.7;">Reminder for your property viewing tomorrow.</p>
            <div style="background:#f8fafc;border:1px solid #e5e7eb;border-radius:14px;padding:20px;line-height:1.8;">
              <strong>${escapeHtml(propertyTitle)}</strong><br />
              Reference: ${escapeHtml(property?.property_ref || "N/A")}<br />
              Date: ${escapeHtml(date)}<br />
    ...
    return NextResponse.json({ success: true, date: tomorrow, bookings: bookings?.length || 0, sent });
  } catch (error) {
    console.error("Viewing reminder cron error:", error);
    return NextResponse.json(
      { success: false, error: "Unable to send viewing reminders." },
      { status: 500 }
    );
  }
}
