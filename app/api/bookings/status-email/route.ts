...
> = {
  ...
  rejected: {
    ...
    message:
      "Unfortunately, we are unable to confirm this viewing request.",
    color: "#b91c1c",
  },
  cancelled: {
  ...
  pending: {
    ...
    message:
      "Your viewing request is currently pending review.",
    color: "#1d4ed8",
  },
};
...
export async function POST(request: Request) {
  try {
    ...
    if (
      ...
      !Object.prototype.hasOwnProperty.call(statusCopy, status)
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    const time = `${String(booking.start_time).slice(
      0,
      5
    ...
    await sendTransactionalEmail({
      ...
      html: emailShell(`
        ...
        <p style="margin:0 0 24px;color:#4b5563;line-height:1.7;">
          Dear ${escapeHtml(booking.customer_name)}, ${escapeHtml(
            copy.message
          )}
        </p>
    ...
    return NextResponse.json(
      {
        success: false,
        error: "Unable to send the booking status email.",
      },
      { status: 500 }
    );
  }
