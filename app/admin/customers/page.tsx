...
import { User, Search, Mail, Phone, ArrowRight, Filter, Hash } from "lucide-react";
...
export default function AdminCustomersPage() {
  ...
  const handleLeadStatusChange = async (customerId: string, newStatus: string) => {
    try {
      ...
      if (error) throw error;
      setCustomers(customers.map(c => c.id === customerId ? { ...c, lead_status: newStatus } : c));
    } catch (err: any) {
  ...
  if (loading) {
    return <div className="p-8 text-center text-gray-500">Loading customers & leads...</div>;
  }
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">Customer Lead Pipeline</h1>
          <p className="text-sm text-gray-500 mt-1">Manage and track customer journeys from initial contact to completion.</p>
        </div>
      </div>
      ...
      <div className="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto pb-2 md:pb-0">
          ...
          {["all", "New", "Contacted", "Viewing Booked", "Application", "Offer", "Completed", "Lost"].map((status) => (
            <button
              key={status}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-gray-100 text-sm">
                {filteredCustomers.map((customer) => {
                  ...
                  return (
                    <tr key={customer.id} className={`hover:bg-gray-50/50 transition-colors ${isInactive ? "bg-red-50/30 opacity-75" : ""}`}>
                      <td className="py-4 px-6">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {isInactive && (
                            <span className="text-[10px] font-bold px-2 py-0.5 bg-red-100 text-red-700 rounded-full">INACTIVE</span>
                          )}
                        </div>
                      </td>
                      <td className="py-4 px-6">
                        <div className="text-gray-900 flex items-center gap-1.5"><Mail className="w-3.5 h-3.5 text-gray-400" /> {customer.email}</div>
                        <div className="text-xs text-gray-500 flex items-center gap-1.5 mt-0.5"><Phone className="w-3.5 h-3.5 text-gray-400" /> {customer.phone || "N/A"}</div>
                      </td>
                      <td className="py-4 px-6">
                        <select
