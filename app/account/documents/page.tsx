...
import { FileText, ArrowLeft, Upload, Clock, CheckCircle2, XCircle, ExternalLink } from "lucide-react";
...
export default function CustomerDocumentsPage() {
  ...
  if (loading) {
    return <div className="min-h-screen bg-gray-50 flex items-center justify-center text-gray-500">Loading documents...</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      <div className="bg-[#1c3053] text-white py-12 px-4 sm:px-8">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-300 text-sm font-light mt-1">Upload and manage your identity and address proofs.</p>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-8 -mt-6 space-y-8">
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-6 sm:p-8">
          ...
          <form onSubmit={handleUpload} className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Document Category</label>
                <select
                  value={category}
              ...
              {category === "Other" && (
                <div>
                  <label className="block text-sm font-medium text-gray-800 mb-1">Specify Document Type</label>
                  <input
                    type="text"
                    required
              ...
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Full Name on Document</label>
                <input
                  type="text"
                  required
                  value={fullName}
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Document / ID Number</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-medium text-gray-800 mb-1">Share Code (Optional)</label>
                <input
                  type="text"
                  value={shareCode}
            ...
            <div>
              <label className="block text-sm font-medium text-gray-800 mb-1">Select File (Image or PDF)</label>
              <input
                type="file"
                required
                accept="image/*,.pdf"
        ...
        <div className="bg-white rounded-2xl shadow-[0_4px_20px_rgb(0,0,0,0.03)] border border-gray-100 p-6 sm:p-8">
          <h2 className="text-xl font-semibold text-gray-900 mb-6">Your Uploaded Documents</h2>
          {documents.length === 0 ? (
            ...
            <div className="space-y-4">
              {documents.map((doc) => (
                <div key={doc.id} className="p-5 rounded-xl border border-gray-200 bg-white shadow-sm flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                  <div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-900">{doc.category}</span>
                      <span className="text-xs text-gray-600 font-medium">({doc.document_number})</span>
                    </div>
                    <p className="text-sm text-gray-800 mt-0.5">Name: {doc.full_name}</p>
                    {doc.share_code && <p className="text-xs text-[#ae884e] font-semibold mt-0.5">Share Code: {doc.share_code}</p>}
                    <p className="text-xs text-gray-500 mt-1">Uploaded: {new Date(doc.created_at).toLocaleDateString("en-GB")}</p>
                  </div>
                  <div className="flex items-center gap-3">
                    ...
                    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-bold ${
                      ...
                      {doc.status === "under_review" && <Clock className="w-3.5 h-3.5" />}
